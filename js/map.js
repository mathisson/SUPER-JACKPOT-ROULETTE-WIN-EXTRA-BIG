...
// A new room (blackjack, one day) is a place here, a model in map3d.js and a case in main.js's go().

import { CasinoMap } from './map3d.js';

export const PLACES = [
  { id: 'slots', emoji: '🐉', name: 'Dragon Rush', color: '#ff2d9a', pos: [-0.4, -3.0], desc: '7×7 tumbling slots, ×1024 spots. DING DING DING.' },
  { id: 'roulette', emoji: '🎡', name: 'Roulette', color: '#2de08f', pos: [-0.4, 2.6], desc: 'The big wheel. Red or black. Mostly black.' },
];
...
export function createMap({ sound, toast, here, go, blocked = () => '', look, daveMet }) {
  ...
  function listHtml() {
    const h = here();
    return PLACES.map(
      (p) => `<button type="button" class="mp-place${p.id === selected ? ' on' : ''}" data-place="${p.id}" style="--c:${p.color}">
        <span>${p.emoji}</span><small>${p.name}</small>${p.id === h ? '<i>YOU</i>' : ''}
      </button>`
    ).join('');
  }
